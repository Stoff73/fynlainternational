<template>
  <div v-if="info" class="bg-white rounded-lg shadow-sm border border-light-gray p-6 mb-6">
    ...
    <!-- Row 2: Financial Overview & Protection Profile -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8 mt-8">
      ...
      <!-- Protection Profile -->
      <div>
        <h3 class="text-body-base font-semibold text-horizon-500 mb-4">Protection Profile</h3>
        <div class="space-y-3">
          <div class="flex justify-between">
            <span class="text-body-sm text-neutral-500">Occupation:</span>
            <span class="text-body-sm text-horizon-500 text-right">{{ info.occupation || '—' }}</span>
          </div>
          <div class="flex justify-between">
          ...
          <div class="flex justify-between">
            ...
            <span class="text-body-sm text-horizon-500 text-right">{{ info.retirement_age != null ? info.retirement_age : '—' }}</span>
          </div>
        </div>
      </div>
    </div>
