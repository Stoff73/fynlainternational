<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-horizon-500">Investment Action Definitions</h2>
        <p class="text-sm text-neutral-500 mt-1">
          Configure the actions that appear in investment plans. Edit thresholds, toggle actions on/off, or create new ones.
        </p>
      </div>
      <button
    ...
    <div v-else class="card p-12 text-center">
      <p class="text-neutral-500">No investment action definitions found. Create one to get started.</p>
    </div>
...
import InvestmentActionModal from './InvestmentActionModal.vue';

export default {
  name: 'AdminInvestmentActions',
  ...
  methods: {
    async fetchDefinitions() {
      ...
      try {
        const response = await adminService.getInvestmentActions();
        this.definitions = response.data.data || [];
      } catch (err) {
        this.error = 'Failed to load investment action definitions.';
      } finally {
        this.loading = false;
      }
    },
    ...
    async handleSave(formData) {
      ...
      try {
        if (this.editingDefinition?.id) {
          await adminService.updateInvestmentAction(this.editingDefinition.id, formData);
          this.showSuccess('Action definition updated successfully.');
        } else {
          await adminService.createInvestmentAction(formData);
    ...
    async toggleAction(def) {
      try {
        const response = await adminService.toggleInvestmentAction(def.id);
    ...
    async executeDelete() {
      ...
      try {
        await adminService.deleteInvestmentAction(this.deleteTarget.id);
