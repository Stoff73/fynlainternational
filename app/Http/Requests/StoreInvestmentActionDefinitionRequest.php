...
class StoreInvestmentActionDefinitionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $uniqueKeyRule = $this->route('id')
            ? Rule::unique('investment_action_definitions', 'key')->ignore($this->route('id'))
            : Rule::unique('investment_action_definitions', 'key');

        return [
            'key' => ['sometimes', 'string', 'max:50', 'regex:/^[a-z0-9_]+$/', $uniqueKeyRule],
            'source' => ['sometimes', 'string', Rule::in(['agent', 'goal'])],
            ...
            'what_if_impact_type' => ['sometimes', Rule::in(['fee_reduction', 'savings_increase', 'contribution', 'tax_optimisation', 'default'])],
