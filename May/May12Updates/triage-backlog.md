...
| E-16 | `TrustController::createTrust/updateTrust` — currency fields use `numeric\|min:0` with no `max:` | G-4-b slice 3 MED M-1 | 2026-05-13 (session 3) | TBD | 8 fields (`initial_value`, `current_value`, `discount_amount`, `loan_amount`, `sum_assured`, `annual_premium`, `retained_income_annual`, `loan_interest_rate`) accept arbitrarily large numbers. Service-layer math crashes or produces meaningless IHT views on overflow. Switch to `ValidationLimits::currencyRules()` / `percentageRules()`. ~30 min + tests. |
| E-17 | `IHTController::storeOrUpdateIHTProfile` — `home_value` / `nrb_transferred_from_spouse` currency overflow | G-4-b slice 3 LOW L-1 | 2026-05-13 (session 3) | TBD | Same shape as E-16. Use `ValidationLimits::currencyRules()`. |
| E-18 | `GiftingController::calculateDiscountedGiftDiscount` + `TrustController::calculateDiscountedGiftDiscount` — `gift_value\|min:1` / `annual_income\|min:0` overflow | G-4-b slice 3 LOW L-2 | 2026-05-13 (session 3) | TBD | Both DGT calculation endpoints accept unbounded currency values, can crash the engine. Migrate to `ValidationLimits::currencyRules()`. |
| E-19 | `TrustController::createTrust/updateTrust` — text fields `beneficiaries`, `trustees`, `purpose`, `notes` accept `nullable\|string` with no `max:` | G-4-b slice 3 LOW L-3 | 2026-05-13 (session 3) | TBD | Large-string DoS candidate. Cap at 5000 chars (or `ValidationLimits::TEXT_FIELD_MAX` if it exists). |
| E-20 | `TrustController` uses inline `$request->validate()` — convention drift | G-4-b slice 3 LOW L-4 | 2026-05-13 (session 3) | TBD | Most pack controllers use FormRequest classes for validation. Cosmetic alignment task; pairs naturally with E-16/E-19. Extract to `StoreTrustRequest` / `UpdateTrustRequest`. |
| E-21 | `FamilyMembersController::index` + `handleSpouseCreation` — PII in INFO-level logs | G-4-b slice 3 LOW L-5 | 2026-05-13 (session 3) | TBD | `Log::info()` lines include user names + emails. Move to `Log::debug()` or obscure email (e.g. `b***@example.com`). Compliance hygiene, not a vulnerability. |
| E-22 | `ReferralController::sendInvitation` — `throttle:10,1` permits 14,400 invites/day per user | G-4-b slice 3 MED M-2 candidate | 2026-05-13 (session 3) | TBD | Spam vector against arbitrary email addresses if `ReferralService::sendInvitation` has no per-target dedup. Verification deferred to G-4-c (service-level audit). Likely fix: per-target idempotency window + tighter throttle. |
| E-23 | `SpouseAccountLinked` mail subject "Your Fynla Account Has Been Linked" no longer matches semantics for the existing-user invite flow | G-4-b slice 3 cosmetic | 2026-05-13 (session 3) | TBD | Post-H-3, existing-user spouse invites send `SpouseDataSharingRequest` (correct). The old `SpouseAccountLinked` is now used only after the invitee accepts. Subject is still technically correct (account IS linked at that point) but worth reviewing copy with UX. |
| E-24 | Unscoped FK IDOR class — `trust_id` (StorePropertyRequest) injects attacker asset into a victim's trust aggregation; `joint_owner_id` across Property/Goal/LifeEvent/Mortgage requests has no spouse/consent check | G-4-b slice 4 MED | 2026-07-06 | TBD | `TrustAssetAggregatorService` filters by `trust_id` only; joint_owner_id contradicts the SpousePermission request→accept design. Durable fix: a shared caller-scoped validation rule object applied wherever a request accepts a linkable/ownable FK (same shape as the S4-H1 fix already shipped for `linked_savings_account_id`). |
| E-25 | Currency fields missing `max:` across ~7/10 sampled requests (property/gift/pension/DC values) | G-4-b slice 4 MED (extends E-16/17/18) | 2026-07-06 | TBD | `ValidationLimits::currencyRules()` exists but is barely used. Prioritise fields feeding IHT/AA/tax calcs. App-wide sweep of `numeric\|min:0` money fields. |
| E-26 | Unbounded text fields (`notes => nullable\|string`, no `max:`) — DoS class | G-4-b slice 4 LOW (extends E-19) | 2026-07-06 | TBD | Constants `MAX_NOTES_LENGTH`/`MAX_DESCRIPTION_LENGTH` exist but unenforced. Project-wide default. |
| E-27 | ZA `ownership_type` accepts `joint` but no `joint_owner_id` field exists on ZA requests | G-4-b slice 4 LOW | 2026-07-06 | TBD | Data-model inconsistency vs GB joint pattern (CLAUDE.md rule 7). Not a security hole. |
| E-28 | ZA exchange-control `amount_minor` / `transfer_date` unbounded | G-4-b slice 4 LOW | 2026-07-06 | TBD | Mitigated by `ExchangeControlLedger::record` throwing 422, but the request layer should cap the amount and bound the date (drives `calendar_year` bucketing). |
| G-4-c-i | Nonsense `tokenable_type` on a corrupted PAT row 500s instead of the ideal 401 | G-4-c LOW (robustness) | 2026-07-06 | TBD | Needs DB write to trigger (attacker already won) and fails closed with no data leak — robustness nit, not an exploit path. Verified by `tests/Feature/Security/MorphEscalationTest.php`. |
